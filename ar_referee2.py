...
COMBO_TIMEOUT = 3.5
VERDICT_FRAMES = 80
HEAD_OFFSET = -20   # wrists must be this many px above nose
...
class BallTracker:
        ...
        for r in results:
            ...
            for box in r.boxes:
                if int(box.cls[0]) != 32:  # sports ball
                    continue
                c = float(box.conf[0])
                if c < BALL_CONF or c <= best_conf:
                    continue
                x1, y1, x2, y2 = box.xyxy[0]
                cx = float((x1 + x2) / 2)
                cy = float((y1 + y2) / 2)
                rad = float(max(x2 - x1, y2 - y1) / 2)
                best, best_conf = (cx, cy, rad), c
    ...
    def _update_state(self):
        ...
        d = math.hypot(recent[-1][0] - recent[0][0],
...
class FormChecker:
    ...
    def reset(self):
        ...
        self.r_wrist = None
        self.nose_y = None
        self.above_shldr = False
        self.above_head = False
    ...
    def update(self, pose_results):
        ...
        # rule 1: both wrists above shoulders
        self.above_shldr = (
            l_wrist and r_wrist and l_shldr and r_shldr and
            l_wrist[1] < l_shldr[1] and r_wrist[1] < r_shldr[1]
        )

        # rule 2: both wrists above head
        self.above_head = (
            l_wrist and r_wrist and nose and
            l_wrist[1] < nose[1] + HEAD_OFFSET and
            r_wrist[1] < nose[1] + HEAD_OFFSET
        )

        # skip first 3 frames (arms still coming up)
        if self._frame <= 3:
            return self.verdict

        if self.above_shldr and self.above_head:
            self._good += 1
            self._bad = 0
            if self._good >= 3:
                self.verdict = "VALID"
...
class Game:
    ...
    def draw(self, frame, ball_tracker, form):
        h, w = frame.shape[:2]
        ...
        # ball trail
        if ball_tracker.history:
            pts = [(int(p[0]), int(p[1])) for p in ball_tracker.history]
            for i in range(1, len(pts)):
                a = i / len(pts)
                cv2.line(frame, pts[i-1], pts[i],
                         (int(220*a), int(200*a), 0), max(1, int(3*a)))
            cv2.circle(frame, pts[-1], 13, (0, 220, 255), 2)
    ...
    def _draw_pose_panel(self, frame, form):
        ...
        for i, (label, ok) in enumerate([
            ("Hands > Shoulders", form.above_shldr),
            ("Hands > Head",      form.above_head),
        ]):
            col = (40, 200, 40) if ok else (40, 40, 220)
            icon = "v" if ok else "x"
    ...
    while True:
        ...
        if game.over:
            ...
            if key == ord('r'):
                ...
                tracker.clear()
                form.reset()
                shot_id, shot_verdict = 0, "UNKNOWN"
                processed_shots.clear()
            continue
        ...
        # new throw started
        if cur_state == BallTracker.FLYING and prev_state != BallTracker.FLYING:
            shot_id += 1
            shot_verdict = form_now   # lock verdict at throw moment
            game.shots += 1
            form.reset()
            print(f"shot #{game.shots}  form={shot_verdict}")

        prev_state = cur_state  # update AFTER the throw detection check

        # hit detection
        if cur_state == BallTracker.FLYING and shot_id not in processed_shots:
            if shot_verdict == "FOUL":
                # register foul once
                game.register_foul()
                processed_shots.add(shot_id)
            elif tracker.history:
                bx, by = tracker.history[-1][0], tracker.history[-1][1]
                for ref in [r for r in game.refs if r.alive]:
                    if ref.hits_ball(bx, by):
                        ref.alive = False
                        ref.flash = 10
                        game.register_hit(ref)
                        processed_shots.add(shot_id)
                        print(f"hit {ref.name}! score={game.score}")
        ...
        game.draw(annotated, tracker, form)

        if frame_idx < 120 and cur_state == BallTracker.ABSENT:
            h2, w2 = annotated.shape[:2]
            cv2.putText(annotated, "raise both hands above your head then throw!",
                        (w2//2-260, h2//2), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (220, 200, 0), 2)
